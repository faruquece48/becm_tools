...
const cumulativeFailed = (row: SummaryRow) => backlogFailedSubjects({ failedSubjects: unique([...row.historicalFailed, ...row.failed]), registerAgain: cumulativeRegister(row) });
...
fetch("/api/students/directory", { cache: "no-store" }).then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.error); return body; }),
  ...
  async function generate() {
    ...
    const legacyFormat = usesLegacyResultFormat(committee.examDate);
    ...
    try {
      ...
      function continuationHeading() { doc.setFont("FreeSerif", "normal"); doc.setFontSize(9); doc.text("Heavens Light is Our Guide", W / 2, 9, { align: "center" }); doc.text("Rajshahi University of Engineering & Technology", W / 2, 13, { align: "center" }); doc.text(`Department of ${departmentName}`, W / 2, 17, { align: "center" }); doc.line(L, 21, R, 21); }
      ...
      const dataPages: SummaryRow[][] = []; let pageRows: SummaryRow[] = [], measuredY = 69; for (const value of summaryRows) { const height = rowHeight(value); if (pageRows.length && measuredY + height > H - 14) { dataPages.push(pageRows); pageRows = []; measuredY = 41; } pageRows.push(value); measuredY += height; } dataPages.push(pageRows); const lastDataStart = dataPages.length === 1 ? 69 : 41, lastDataEnd = dataPages[dataPages.length - 1].reduce((position, value) => position + rowHeight(value), lastDataStart), statisticsOnNewPage = lastDataEnd + 7 + 33 > H - 14; let y = 0, currentPage = dataPages.length;
      dataPages.forEach((items, pageIndex) => { if (pageIndex > 0) doc.addPage("a4", "portrait"); if (pageIndex === 0) { doc.setFont("FreeSerif", "normal"); doc.setFontSize(10.5); doc.text(`Date of Examination: ${formatTabulatorDate(committee.examDate)}`, R, 19, { align: "right" }); doc.setFont("FreeSerif", "normal"); doc.text("Heavens Light is Our Guide", W / 2, 24, { align: "center" }); doc.text("Rajshahi University of Engineering & Technology", W / 2, 29, { align: "center" }); doc.text(`Department of ${departmentName}`, W / 2, 34, { align: "center" }); doc.setFont("FreeSerif", "normal"); const introduction = `Subject to the approval of the Syndicate on recommendation of the Academic Council, the result of B.Sc Engineering ${ordinal[selection.academicYear]} Year ${selection.semester} Semester Examination, ${selection.examYear} is published as follows-`; const introductionLines: string[] = doc.splitTextToSize(introduction, R - L); doc.text(introductionLines, L, 42, { align: "justify", maxWidth: R - L, lineHeightFactor: 1.15 }); y = tableHeader(53); } else { continuationHeading(); y = tableHeader(25); } items.forEach((value) => { y = row(y, value); }); if (pageIndex < dataPages.length - 1 || statisticsOnNewPage) footer(pageIndex + 1, totalPagesToken); });
