...
export default function NonObeExamTable({ selection }: { selection: Selection; title: string }) {
  ...
  const cohort = useMemo(() => students
    ...
    .sort((left, right) => left.rollNo.localeCompare(right.rollNo, undefined, { numeric: true })), [students, selection]);
  ...
  function result(student: OldStudentRecord, course: SyllabusCourse) {
    ...
    const total = mark ? Math.round(theory ? (mark.present ? numberValue(mark.partA) + numberValue(mark.partB) : 0) + numberValue(mark.classTestAttendance) : numberValue(mark.sessional) + viva) : 0;
  ...
  return <div className="mt-10 pt-2">
    ...
    <SyncedHorizontalScroll><table className="min-w-max border-collapse text-sm">
      ...
      <tr>{courses.flatMap((course) => (course.type === "Theory" ? ["A", "B", "CT", "T.", "Gr."] : ["Ses", "T.", "Gr."]).map((label) => <th key={`${course.id}-${label}`} className="border p-3">{label}</th>))}<th className="border p-3">Failed Subjects</th><th className="border p-3">Need to Register Again</th></tr></thead>
