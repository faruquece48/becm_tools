...
import { ArrowRight, Bell, BookOpenText, CalendarDays, FileText, Grid2X2, LogOut, Menu, Settings, Users, X, type LucideIcon } from "lucide-react";
import logoImage from "@/app/images/image_03.png";

type DashboardItem = { title: string; description: string; icon: LucideIcon };
type Props = { role: "Student" | "Staff"; subtitle: string; welcome: string; items: DashboardItem[]; accent: string };

export default function RoleDashboard({ role, subtitle, welcome, items, accent }: Props) {
  ...
  const sidebar = <aside className="flex h-screen w-[var(--app-sidebar-width)] shrink-0 flex-col border-r border-[#12396d] bg-[#082452] text-white">
    ...
    <nav className="space-y-1 px-4 py-4" aria-label={`${role} navigation`}>
      {navigation.map(({ label, icon: Icon, active }) => <span key={label} aria-current={active ? "page" : undefined} aria-disabled={!active} title={active ? undefined : "This page is coming soon"} className={`flex w-full items-center gap-4 rounded-xl px-5 py-3 text-sm font-medium ${active ? `bg-gradient-to-r ${accent} text-white shadow-lg shadow-blue-950/40` : "cursor-not-allowed text-blue-200/50"}`}><Icon className="h-5 w-5" /><span>{label}</span></span>)}
    </nav>
