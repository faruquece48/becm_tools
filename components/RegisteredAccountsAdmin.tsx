...
export default function RegisteredAccountsAdmin({ role, title }: { role: Role; title: string }) {
  ...
  return <div>
    ...
    {loading ? <div className="flex h-64 items-center justify-center"><LoaderCircle className="h-8 w-8 animate-spin text-blue-600" /></div> : <div className="mt-5 overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm"><table className="w-full min-w-[1280px] table-auto text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="min-w-[310px] whitespace-nowrap px-5 py-4">Account</th><th className="min-w-[130px] whitespace-nowrap px-5 py-4">Registered</th><th className="min-w-[190px] whitespace-nowrap px-5 py-4">Last login</th><th className="whitespace-nowrap px-5 py-4">Logins</th><th className="min-w-[120px] whitespace-nowrap px-5 py-4">Status</th><th className="min-w-[430px] whitespace-nowrap px-5 py-4">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((account) => <tr key={account.id}><td className="whitespace-nowrap px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-50 text-blue-700"><UsersRound className="h-5 w-5" /></span><div><p className="font-semibold">{account.name || "Unnamed account"}</p>{account.passwordResetRequestedAt && <p className="mt-1 inline-block rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-800">Password reset requested</p>}<p className="text-slate-500">{account.email}{account.phone ? ` Ãƒâ€šÃ‚Â· ${account.phone}` : ""}</p></div></div></td><td className="whitespace-nowrap px-5 py-4">{new Date(account.registeredAt).toLocaleDateString()}</td><td className="whitespace-nowrap px-5 py-4">{account.loginCount ? new Date(account.lastLoginAt).toLocaleString() : "Never"}</td><td className="whitespace-nowrap px-5 py-4 font-semibold">{account.loginCount}</td><td className="whitespace-nowrap px-5 py-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${account.active ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{account.active ? "Active" : role === "student" && account.loginCount === 0 ? "Pending approval" : "Disabled"}</span></td><td className="whitespace-nowrap px-5 py-4"><button type="button" onClick={() => void toggle(account)} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold ${account.active ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>{account.active ? <ShieldOff className="h-4 w-4" /> : <ShieldCheck className="h-4 w-4" />}{account.active ? "Disable" : role === "student" && account.loginCount === 0 ? "Approve" : "Enable"}</button><button type="button" onClick={() => void resetPassword(account)} className="ml-2 inline-flex items-center gap-1 rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700"><KeyRound className="h-4 w-4" />Reset Password</button><button type="button" onClick={() => void editAccount(account)} className="ml-2 inline-flex items-center gap-1 rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700"><Pencil className="h-4 w-4" />Edit</button><button type="button" onClick={() => void deleteAccount(account)} className="ml-2 inline-flex items-center gap-1 rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-700"><Trash2 className="h-4 w-4" />Delete</button></td></tr>)}{!filtered.length && <tr><td colSpan={6} className="px-5 py-12 text-center text-slate-500">No {role} accounts found.</td></tr>}</tbody></table></div>}
  </div>;
}
