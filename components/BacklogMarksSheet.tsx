...
import { statusInsteadOfFailedSubjects, usesLegacyResultFormat } from "@/lib/resultFormatPolicy";
...
export default function BacklogMarksSheet({ examType, onExamTypeChange }: { examType: "Regular" | "Backlog"; onExamTypeChange: (value: "Regular" | "Backlog" | "Short Semester") => void }) {
...
fetch("/api/students/directory", { cache: "no-store" }).then((response) => response.json()),
      ...
      loadExamCommittees(),
    ]).then(([studentBody, oldStudentBody, syllabusBody, savedMarks, savedArchives, registrationBody, committeeRows]) => {
  ...
  const legacyFormat = usesLegacyResultFormat(committees.find((item) => item.examType === "Backlog" && item.examYear === selection.examYear && item.academicYear === selection.academicYear)?.examDate);
