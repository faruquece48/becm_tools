...
export default function BacklogMarksSheet() {
  ...
  })).values()].sort((left, right) => (left.semester === "Odd" ? 0 : 1) - (right.semester === "Odd" ? 0 : 1) || left.code.localeCompare(right.code, undefined, { numeric: true })), [currentMarks, syllabuses, selection.academicYear]);
  ...
  async function generatePdf() {
    ...
    const drawSection = (label: string, studentsInSection: SheetStudent[]) => { if (!studentsInSection.length) return; const rows = rowData(studentsInSection); if (y + 4 + 18 + rowHeight(rows[0]) > H - 14) nextPage(); doc.setFont("FreeSerif", "bold"); doc.setFontSize(8); doc.text(`${label}:`, L, y + 3); y = tableHeader(y + 5); rows.forEach(({ student, data }, index) => { const height = rowHeight({ student, data }); if (y + height > H - 14) { nextPage(); doc.setFont("FreeSerif", "bold"); doc.setFontSize(8); doc.text(`${label} (continued):`, L, y + 3); y = tableHeader(y + 5); } const values = data.rows; let x = L; cell(x, y, rollWidth, height, student.rollNo); x += rollWidth; values.forEach(({ mark, letter }) => { [mark?.partA ?? "-", mark?.partB ?? "-", mark?.classTestAttendance ?? "-", mark?.marks ?? "-", letter].forEach((value, valueIndex) => { cell(x, y, subWidth, height, String(value), false, "center", 7.3, valueIndex === 4 && letter === "F"); x += subWidth; }); }); cell(x, y, earnedWidth, height, data.earned.toFixed(2)); x += earnedWidth; cell(x, y, sgpaWidth, height, data.sgpa.toFixed(2)); x += sgpaWidth; cell(x, y, failedWidth, height, data.failed.join(", "), false, "left", 7.3, data.failed.length > 0); cell(x + failedWidth, y, registerWidth, height, "", false, "left"); y += height; if (index === rows.length - 1) y += 6; }); };
    heading(); drawSection("OBE", obeCohort); drawSection("Non-OBE", nonObeCohort); footer(); doc.putTotalPages(totalPagesToken);
