...
export function flattenAssignment(
  courses: CourseDuty[],
    ...
    course.parts.forEach((part) => {
      ...
      entries.forEach((entry) => {
        ...
        rows.push({
          ...
          // The assignment list shows the full batch size for every assigned
          // teacher/part. The individual bill calculation separately applies
          // the teacher share (for example, 27 / 2).
          assignmentValue: totalStudents === "" ? "" : String(totalStudents),
        });
      });
    });
  });
