...
const SECTION = "old-student-result-updates";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ records: Array.isArray(rows[0]?.data) ? rows[0].data : [] }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    console.error("Unable to load special-student result history", error);
    return NextResponse.json({ error: "Unable to load special-student result history" }, { status: 503 });
  }
}
