 ...
 try{
  const result=await p.$transaction(async tx=>{
   ...
   for(const published of (Array.isArray(publications)?publications:[]) as Array<Record<string,unknown>>){
    ...
    const belongs=(student:Record<string,unknown>)=>student.id===item.id||student.studentId===item.id||String(student.rollNo||"").trim()===item.rollNo.trim();
    const cohort=Array.isArray(published.students)?published.students as Array<Record<string,unknown>>:[];
    const ledgerData=rows.find(row=>row.section==='old-student-result-updates')?.data;
    const ledger=Array.isArray(ledgerData)?ledgerData as Array<Record<string,unknown>>:[];
    const hasPublishedResult=cohort.some(belongs)||ledger.some(entry=>entry.applied===true&&entry.examYear===exam.examYear&&entry.academicYear===exam.academicYear&&entry.examType===exam.examType&&(exam.examType==="Backlog"||entry.semester===exam.semester)&&Array.isArray(entry.students)&&(entry.students as Array<Record<string,unknown>>).some(belongs));
    if(hasPublishedResult&&promotionCoursesChanged(previous?.promotions||[],item.promotions,exam))return {error:`This student has a published result for ${exam.examYear}, ${exam.academicYear} Year ${exam.examType==="Backlog"?"Backlog":exam.semester}. Send that result back before changing its promotion.`,status:409};
   }
