...
export async function GET(request: Request) {
  ...
  const orders = await prisma.rentalOrder.findMany({ where: { studentEmail: email, status: { in: ["ACTIVE", "RETURNED"] } }, include: { items: { include: { book: true } } }, orderBy: { createdAt: "desc" } });
